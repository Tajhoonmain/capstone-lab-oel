    ...
    if name == "calculate_gpa":
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Successfully registered for {course_code} ({course_name}) - {credits} credits - {semester}. Registration ID: {registration_id}",
                )
            ],
            isError=False,
        )
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Grade prediction for {course_code} ({course_name}): Current {current_grade} → Predicted Final: {round(predicted_final, 2)} (assuming 85 on final exam)",
                )
            ],
            isError=False,
        )
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Assignment tracked: '{assignment_name}' for {course_code} ({course_name}). Due: {due_date}. Status: {status}. Tracking ID: {tracking_id}",
                )
            ],
            isError=False,
        )
        ...
        info = students.get(
            student_id, {"name": "Unknown Student", "major": "Undeclared", "year": "Freshman", "gpa": 0.0, "credits": 0}
        )
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Student Info: {info['name']} - {info['major']} ({info['year']}) - GPA: {info['gpa']}, Credits: {info['credits']}",
                )
            ],
            isError=False,
        )
        ...
        return types.CallToolResult(
            content=[
                types.TextContent(
                    type="text",
                    text=f"Credits needed for {major} graduation: {needed} more credits required (out of {required} total). Estimated {semesters_needed} semesters remaining to graduate by {target_year}.",
                )
            ],
            isError=False,
        )
