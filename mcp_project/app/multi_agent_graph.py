...
def researcher_node(state: AgentState):
    ...
    model = ChatGoogleGenerativeAI(
        model=MODEL_NAME, google_api_key=os.getenv("GEMINI_API_KEY"), temperature=0
...
def advisor_node(state: AgentState):
    ...
    context_msgs.append(
        HumanMessage(
            content="[SYSTEM]: The Researcher has finished gathering information. Advisor, please formulate the final response for the student using your tools if needed."
        )
    )
